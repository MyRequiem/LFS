...
# Набор заголовочных файлов на языке C++, который предоставляет программистам
# математические функции для работы с 3D-графикой. Он полностью копирует логику
# работы шейдеров OpenGL.
