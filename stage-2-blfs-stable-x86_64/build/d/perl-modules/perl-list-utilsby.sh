...
PRGNAME="perl-list-utilsby"
ARCH_NAME="List-UtilsBy"

### List::UtilsBy (higher-order list utility functions)
# List::UtilsBy Perl модуль
...
sed -e "s|${TMP_DIR}||" -i \
    "${TMP_DIR}${PERL_LIB_PATH}/site_perl/auto/List/UtilsBy/.packlist"

source "${ROOT}/stripping.sh"      || exit 1
source "${ROOT}/update-info-db.sh" || exit 1
/bin/cp -vpR "${TMP_DIR}"/* /
cat << EOF > "/var/log/packages/${PRGNAME}-${VERSION}"
# Package: ${PRGNAME} (higher-order list utility functions)
#
# List::UtilsBy provides a number of higher-order list utility functions
#
# Home page: https://metacpan.org/pod/List::UtilsBy
# Download:  https://cpan.metacpan.org/authors/id/P/PE/PEVANS/${ARCH_NAME}-${VERSION}.tar.gz
#
EOF
