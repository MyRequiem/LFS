...
# Служебная библиотека, которая считывает скрытые метаданные из графических
# файлов (например, дату съемки, настройки камеры или модель телефона).
# Файловые менеджеры (например, pcmanfm-qt) используют её главным образом для
# того, чтобы автоматически поворачивать миниатюры фотографий в папках
